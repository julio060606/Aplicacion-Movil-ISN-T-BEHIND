...
## 3. ARQUITECTURA DE INTERACCIONES DEL TURNO (Color Boxes & 8 Sistemas)

> [!IMPORTANT]
> El sistema anterior de "Split 40/60 con Flexbox" fue reemplazado. Ahora todos los elementos se posicionan de forma absoluta usando coordenadas de Figma escaladas al dispositivo.

Las 4 zonas Color Box definen regiones visuales (no contenedores físicos):
...
state "🔴 ZONA A — ROJO (10.71%) Y:0→156" as ZonaA {
    state "roll-up-door" as RUD : PanResponder vertical\nSnap arriba (cerrado) / abajo (634px = fin amarillo)\nDrag lento → spring a posición más cercana
}

state "🟡 ZONA B — AMARILLO (32.83%) Y:156→634" as ZonaB {
    state "npc-1 (cliente)" as NPC : fade al 25% en inspección
    state "rejilla" as Reja : fade al 25% en inspección
    state "Inspección táctil" as Inspect : Touch en zona B\n→ Zoom 1.25×, pan horizontal ±60px
    state "lampara-cabeza" as Lamp : PanResponder drag libre\n±80px H / ±60px V\nSwipe rápido → reset con spring
}

state "🩵 ZONA C — CELESTE (22.12%) Y:634→956" as ZonaC {
    state "pc" as PC : Tap → overlay MONITOR\n(zoom 0.88→1, dim 72%)
    state "carrusel-mesa" as Carousel : Tap → zoom cámara 1.28×\nFlechas ❮❯ navegan ítems\nBotón [COBRAR] aparece al enfocar
}

state "🔵 ZONA D — AZUL (34.34%) Y:956→1456" as ZonaD {
    state "cajon-1 / cajon-2" as Cajones : Tap → abrir/cerrar\nFlechas ❮❯ navegan ítems del cajón
    state "caja-carton" as CajaC : Tap → submenu (placeholder)
}

state "OVERLAYS SUPREMOS (fuera del flujo normal)" as Overlays {
    state "APUNTAR (zIndex 999)" as Aim : Bloquea TODA la pantalla\nSolo [GUARDAR] o [DISPARAR]
    state "MONITOR OVERLAY (zIndex 800)" as Mon : Tap fuera → cerrar\nDim 72% fondo
    state "CAJA REGISTRADORA (zIndex 800)" as Caja : Slide desde abajo\nCalculadora de vuelto (pendiente)\nTap fuera → cerrar
}

ZonaA -down-> ZonaB : Los sprites pueden cruzar zonas\n(overflow: visible)
ZonaB -down-> ZonaC : scale = windowHeight / 1456
ZonaC -down-> ZonaD
Carousel --> Caja : Botón [COBRAR]\nabre registradora
Cajones --> Aim : Hold cajón derecho\nextre revólver (pendiente)
PC --> Mon : Tap
...
| Condición activa | Qué bloquea |
|---|---|
| `isAiming = true` | Todo excepto [GUARDAR] / [DISPARAR] |
| `monitorOpen = true` | Todo excepto tap fuera del monitor |
| `cajaOpen = true` | Todo excepto botones de la caja y tap fuera |
| `carouselFocused = true` | Inspección táctil (Zona B); puede abrir caja |
