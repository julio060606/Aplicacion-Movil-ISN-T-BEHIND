...
> [!IMPORTANT]
> Las referencias están ordenadas por **prioridad de influencia**. La Referencia 3 es el punto de partida canónico para todos los sprites del gameplay.

---

### 🥇 Referencia 3 — PRIORIDAD MÁXIMA (canon visual del juego)
**Archivo:** `docs/ref_estetica_03.jpg`

La imagen que más se acerca a la visión final. Es una fotografía real tomada en una **tienda/minimarket** con estanterías de productos al fondo. La persona es prácticamente una silueta negra sin rasgos discernibles. El entorno (estanterías, letreros, productos, envases) **se reconoce perfectamente** a pesar de la posterización. Texto cirílico legible ("ОСТОРОЖНО, ГОРЯЧО!", "Здесь"). Un objeto rojo-naranja abajo a la derecha destaca como acento.

**Por qué es la referencia principal:**

1. **Es literalmente el mismo espacio que el juego.** Una tienda con estanterías de productos, mostrador, letreros — exactamente el mundo del protagonista. No hay que imaginar cómo aplicar el efecto al contexto; ya está aplicado.
2. **Demuestra que los objetos de tienda se reconocen.** Botellas, cajas, envases, muebles — todo legible con solo 3 tonos. Si funciona en esta foto, funcionará en nuestros renders.
3. **El negro domina (~60%).** No es el azul lo que domina — es el negro. El color aparece como acento. Esto es exactamente la atmósfera opresiva que buscamos.
4. **Los textos/letreros sobreviven.** Los carteles del C.S.T. en las paredes de la tienda serán legibles de la misma manera.
5. **La persona como silueta.** El cliente/sujeto no tiene rostro distinguible — es una mancha oscura. Deshumanización perfecta por el propio efecto.

**Características técnicas a replicar:**

| Parámetro | Valor observado |
|---|---|
| **Distribución tonal** | Negro ~60% / Azul petróleo oscuro ~30% / Azul CST saturado ~10% |
| **Posterización** | 3 niveles, agresiva |
| **Grano** | Medio-bajo (visible pero no dominante) |
| **Legibilidad objetos** | Alta — las siluetas se leen, el texto se lee |
| **Legibilidad personas** | Baja/Nula — silueta oscura sin rasgos |
| **Contraste de bordes** | Duro. Negro puro junto a azul saturado, sin transición suave |
| **Nivel Gradient Map medios** | Posición 40-45% (más negro que azul) |

**Este es el aspecto de:** fondos de tienda, estanterías, objetos de fondo, el exterior visible por la ventanilla. Es también el aspecto base del gameplay.

---

### 🥈 Referencia 1 — PRIORIDAD ALTA (objetos interactivos en primer plano)
**Archivo:** `docs/ref_estetica_01.jpg`

Persona sentada contra una pared con una botella en la mano. Posterización de 3 niveles limpia. Bordes duros. Silueta perfectamente legible. **Más azul saturado visible** que en la Referencia 3.

**Características técnicas:**

| Parámetro | Valor observado |
|---|---|
| **Distribución tonal** | Negro ~45% / Azul petróleo oscuro ~20% / Azul CST saturado ~35% |
| **Posterización** | 3 niveles, agresiva pero con más detalle que Ref.3 |
| **Grano** | Sutil (5-8%) |
| **Legibilidad** | Alta — el color saturado ilumina bien la forma |
| **Contraste** | Extremo. Los bordes son muy nítidos. |

**Diferencia clave vs Referencia 3:** Aquí el color saturado (azul CST) ocupa mucho más espacio. Esto sirve para objetos que el jugador DEBE notar: elementos interactivos en primer plano (caja registradora, cuaderno, sellos, documentos). El azul brillante atrae la atención.

**Este es el aspecto de:** objetos interactivos en primer plano, elementos de UI de gameplay que el jugador debe identificar rápidamente.

---

### 🥉 Referencia 2 — ESTADO DE ESTRÉS / ABSTINENCIA
**Archivo:** `docs/ref_estetica_02.jpg`

Persona con pelo largo en entorno oscuro. Mayor cantidad de grano analógico grueso. Los bordes se pierden en el ruido. Formas más difusas.

**Características técnicas:**

| Parámetro | Valor observado |
|---|---|
| **Distribución tonal** | Negro ~70% / Azul petróleo ~25% / Azul CST ~5% |
| **Grano** | Pesado, grueso (15-25%), dominante |
| **Legibilidad** | Baja — formas se disuelven en el ruido |
| **Bordes** | Suavizados por el grano |

**Este NO es el estado base del gameplay.** Es el efecto que se aplica dinámicamente en runtime cuando el estrés/abstinencia del protagonista sube. A mayor nivel de estrés → progresión hacia la Referencia 2.

---

### Tabla Comparativa: Gradiente de Estado

| Aspecto | Ref. 3 — Estado Base | Ref. 1 — Objetos Interactivos | Ref. 2 — Estrés/Abstinencia |
|---|---|---|---|
| Negro dominante | ~60% | ~45% | ~70% |
| Azul CST visible | Acento ~10% | Iluminación ~35% | Mínimo ~5% |
| Grano | Medio-bajo | Sutil | Pesado |
| Legibilidad | Alta (entorno) | Alta (objeto) | Baja (horror) |
| Bordes | Duros | Muy duros | Difusos |
| Uso principal | **Fondo + atmósfera** | **Primer plano interactivo** | **Overlay de estrés** |
...
PASO 5: MAPEAR COLORES (Gradient Map / Color Lookup)
    │  
    │  ─── PRESET A: Fondos y atmósfera (estilo Referencia 3 — PREDETERMINADO) ───
    │  Crear un Gradient Map con 3 paradas:
    │  - Posición 0%  (sombras): #000000 (Negro Absoluto)
    │  - Posición 42% (medios):  #1A2A3A (Azul Petróleo Oscuro)  ← más negro
    │  - Posición 100% (luces):  #2E86C1 (Azul C.S.T.)
    │  Resultado: negro ~60%, azul oscuro ~30%, azul CST solo en acentos.
    │  
    │  ─── PRESET B: Objetos interactivos en primer plano (estilo Referencia 1) ───
    │  - Posición 0%  (sombras): #000000 (Negro Absoluto)
    │  - Posición 50% (medios):  #1A2A3A (Azul Petróleo Oscuro)  ← equilibrado
    │  - Posición 100% (luces):  #2E86C1 (Azul C.S.T.)
    │  Resultado: negro ~45%, azul oscuro ~20%, azul CST ~35% (más visible).
    │
    ▼
PASO 6: AÑADIR GRANO (varía según capa)
    │  Añadir capa de ruido monocromático.
    │  (Filter → Noise → Add Noise → Monochromatic)
    │  
    │  PRESET A (fondos/atmósfera — Ref.3): 8-12% de ruido
    │  PRESET B (objetos interactivos — Ref.1): 5-8% de ruido
    │  ESTRÉS/ABSTINENCIA (se aplica en runtime): 15-30% de ruido
    │
    ▼
...
> **Regla rápida:** ¿El elemento está en el fondo o es parte del ambiente? → **Preset A**. ¿El jugador debe tocar o interactuar con él? → **Preset B**.
